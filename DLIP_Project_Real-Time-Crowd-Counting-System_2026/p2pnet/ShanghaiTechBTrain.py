...
# Absolute paths to ShanghaiTech Part B list files
_REPO_ROOT = Path(__file__).resolve().parent.parent
_PART_B_TRAIN = str(_REPO_ROOT / 'data' / 'shanghaitech_p2p' / 'partB_train.list')
_PART_B_TEST  = str(_REPO_ROOT / 'data' / 'shanghaitech_p2p' / 'partB_test.list')


def get_args_parser():
    parser = argparse.ArgumentParser('P2PNet training – ShanghaiTech Part B', add_help=False)
    ...
    # Dataset – Part B defaults
    parser.add_argument('--dataset_file', default='SHHA')
    parser.add_argument('--data_root', default='./new_public_density_data')
    parser.add_argument('--train_list', default=_PART_B_TRAIN)
    parser.add_argument('--val_list',   default=_PART_B_TEST)

    # Output – separate dirs so Part B weights never overwrite Part A
    parser.add_argument('--output_dir',      default='./log_partB')
    parser.add_argument('--checkpoints_dir', default='./ckpt_partB')
    parser.add_argument('--tensorboard_dir', default='./runs_partB')
...
def main(args):
    ...
    with open(run_log_name, "a") as log_file:
        log_file.write('\n[ShanghaiTech Part B] Eval Log %s\n' % time.strftime("%c"))
    ...
    print("Start training – ShanghaiTech Part B")
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch % args.eval_freq == 0 and epoch != 0:
            ...
            if current_mae < best_mae - args.min_delta:
                ...
                # Best checkpoint saved as partB_best_mae.pth
                torch.save({
                    'model': model_without_ddp.state_dict(),
                    'optimizer': optimizer.state_dict(),
                    'lr_scheduler': lr_scheduler.state_dict(),
                    'epoch': epoch,
                }, os.path.join(args.checkpoints_dir, 'partB_best_mae.pth'))
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser('P2PNet Part B training', parents=[get_args_parser()])
