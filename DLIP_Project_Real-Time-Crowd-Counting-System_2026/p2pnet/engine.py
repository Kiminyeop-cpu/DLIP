...
def evaluate_crowd_no_overlap(model, data_loader, device, vis_dir=None):
    ...
    maes = []
    mses = []
    rel_errs = []  # per-image |pred-gt|/gt (gt>0 only)
    for samples, targets in data_loader:
        samples = samples.to(device)
        ...
        # 상대오차: gt=0이면 제외 (분모 0 방지)
        if gt_cnt > 0:
            rel_errs.append(abs(predict_cnt - gt_cnt) / gt_cnt * 100.0)

    mae = np.mean(maes)
    mse = np.sqrt(np.mean(mses))
    # mean relative MAE % (gt>0 이미지만)
    rel_mae_pct = np.mean(rel_errs) if rel_errs else 0.0

    return mae, mse, rel_mae_pct